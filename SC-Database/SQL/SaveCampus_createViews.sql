...
SELECT 
    ...
    m.expires_at,
    (SELECT COUNT(*) FROM Claims c WHERE c.meal_id = m.meal_id) AS total_claims
FROM Users u, Meals m
WHERE m.staff_id = u.user_id
    AND u.role = 'staff';
GO
...
SELECT 
    ...
    m.name AS meal_name,
    m.expires_at
FROM Claims c, Users u, Meals m
WHERE u.user_id = c.student_id
    AND m.meal_id = c.meal_id
    AND u.role = 'student';
GO
...
SELECT 
    ...
    u.full_name AS staff_name
FROM Meals m, Users u
WHERE u.user_id = m.staff_id
    AND (m.available_portions <= 0
