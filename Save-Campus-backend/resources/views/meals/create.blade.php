                    ...
                    <form method="POST" action="{{ route('meals.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-sm text-gray-500">Optional: Provide details about ingredients, allergens, etc.</p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            @error('image_path')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-sm text-gray-500">Optional: Upload a photo of the meal (PNG, JPG, GIF)</p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="available_portions" id="available_portions" value="{{ old('available_portions') }}" min="1"
                        ...
                        <div class="mb-6">
                            ...
                            @error('expires_at')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-1 text-sm text-gray-500">When will this meal no longer be available?</p>
                        </div>

                        <!-- Form Actions -->
                        <div class="flex items-center justify-end gap-4">
                            <a href="{{ route('meals.index') }}" class="inline-flex items-center px-6 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-400 focus:bg-gray-400 active:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Cancel
                            </a>
                            <button type="submit" class="inline-flex items-center px-6 py-2 bg-gradient-to-r from-green-600 to-emerald-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:shadow-lg hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Create Meal
                            </button>
                        </div>
                    </form>
