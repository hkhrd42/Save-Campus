...
class ClaimController extends Controller
{
    ...
    public function index()
    {
        ...
        // Calculate dashboard metrics efficiently using database queries
        $userId = auth()->id();
        $totalClaims = \DB::table('claims')->where('user_id', $userId)->count();
        
        $activeClaims = \DB::table('claims')
            ->join('meals', 'claims.meal_id', '=', 'meals.id')
            ->where('claims.user_id', $userId)
            ->where('meals.expires_at', '>', now())
            ->count();
