...
class MealController extends Controller
{
    ...
    public function index()
    {
        ...
        // Calculate dashboard metrics efficiently using database aggregates
        $mealsQuery = Meal::where('user_id', auth()->id());
        $totalMeals = $mealsQuery->count();
        $totalPortions = (clone $mealsQuery)->sum('available_portions');
        $totalClaims = \DB::table('claims')
            ->whereIn('meal_id', (clone $mealsQuery)->pluck('id'))
            ->count();
